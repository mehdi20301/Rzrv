...
using RZRV.Migrations.Seed.Host;
...
namespace RZRV.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : RZRVAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<RZRVDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{RZRVConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{RZRVConsts.ConnectionStringName}", constring);
        }
